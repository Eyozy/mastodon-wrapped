...
const getColorConfig = (count) => {
  ...
  return (
    HEATMAP_LEVELS.find((level) => count >= level.min) ||
    HEATMAP_LEVELS[HEATMAP_LEVELS.length - 1]
  );
};
...
export default function ActivityHeatmap({ activityCalendar, year, t }) {
  ...
  const handleMouseOver = useCallback(
    (event, value) => setHoveredData(value),
    []
  );
